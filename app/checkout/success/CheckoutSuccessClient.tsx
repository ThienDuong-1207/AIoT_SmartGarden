...
export default function CheckoutSuccessClient() {
  ...
  const [orderNumber] = useState(() => buildOrderNumber());
  const searchParams = useSearchParams();
  const orderFromQuery = searchParams.get("order");
  const orderCode = orderFromQuery || orderNumber;
      ...
      >
        ...
        <div className="p-8">
          ...
          >
            ...
            <span className="font-mono text-sm font-bold" style={{ color: "var(--emerald-400)" }}>
              {orderCode}
            </span>
          </div>
            ...
            {STEPS.map(({ icon: Icon, label, desc, done }, i) => (
              <div key={label} className="flex gap-4">
                ...
                <div className="pb-4 pt-1">
                  <p className="text-xs font-semibold" style={{ color: done ? "var(--text-primary)" : "var(--text-muted)" }}>
                    {label}
                  </p>
